...
def demonstrate_sorting():
    ...
    try:
        ...
        print(f"Error while sorting records: {exc}")
        return
...
def demonstrate_reports():
    ...
    try:
        ...
        print(f"\nError generating summary report: {exc}")

    try:
        top_products = reporter.generate_top_products_report(3)
        print("\n" + top_products)
    except Exception as exc:
        print(f"\nError generating top products report: {exc}")

    try:
        regional = reporter.generate_regional_report()
        print("\n" + regional)
    except Exception as exc:
        print(f"\nError generating regional report: {exc}")
...
def demonstrate_calculations():
    ...
    try:
        ...
        print(
            f"Profit Margin (Revenue: $10,000, Costs: $6,000): "
            f"{profit_margin:.2f}%"
        )
    except Exception as exc:
        print(f"Error calculating profit margin: {exc}")

    try:
        roi = BusinessCalculator.calculate_roi(15000, 10000)
        print(f"ROI (Gain: $15,000, Cost: $10,000): {roi:.2f}%")
    except Exception as exc:
        print(f"Error calculating ROI: {exc}")
    ...
    try:
        ...
        print(f"Error calculating compound growth rate: {exc}")
    ...
    try:
        ...
        print(f"Error calculating break-even point: {exc}")
...
def demonstrate_parallel_processing():
    ...
    try:
        ...
        print(f"Error during parallel processing: {exc}")
        return
